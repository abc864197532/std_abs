...
struct LCT {
    ...
    void pull(int i) {
        ...
        sz[i] = 1;
        if (ch[i][0])
            sum[i] += sum[ch[i][0]], sz[i] += sz[ch[i][0]];
        if (ch[i][1])
            sum[i] += sum[ch[i][1]], sz[i] += sz[ch[i][1]];
    }
    void push(int i) {
        if (lz_mul[i] != 1)
            mul(ch[i][0], lz_mul[i]), mul(ch[i][1], lz_mul[i]), lz_mul[i] = 1;
        if (lz_add[i])
            add(ch[i][0], lz_add[i]), add(ch[i][1], lz_add[i]), lz_add[i] = 0;
        if (rev[i])
            reverse(ch[i][0]), reverse(ch[i][1]), rev[i] = false;
    }
    void reverse(int i) {
        if (!i)
            return;
    ...
    void splay(int i) {
        ...
        while (!isrt(i)) {
            int p = pa[i];
            if (!isrt(p)) {
                rotate(ch[p][1] == i ^ ch[pa[p]][1] == p ? i : p);
            }
            rotate(i);
        }
    }
