...
struct seg {
    ...
    void push() {
        if (lz)
            ch[0]->add(l, r, lz), ch[1]->modify(l, r, lz), lz = 0;
    }
    void pull() {
...
struct seg2 {
    ...
    seg2(int _l, int _r) : l(_l), r(_r), v(0, N), lz(0, N) {
        if (l < r - 1)
            ch[0] = new seg2(l, l + r >> 1), ch[1] = new seg2(l + r >> 1, r);
    }
    ...
    ll qsum(int _l, int _r, int _l2, int _r2) {
        ...
        if (_l < l + r >> 1)
            res += ch[0]->query(_l, _r, _l2, _r2);
        if (l + r >> 1 < _r)
            res += ch[1]->query(_l, _r, _l2, _r2);
        return res;
    }
};
